...
Having done the required subcircuits, we now proceed with the design of the multiplier itself. The design comprises an array of AND Gates arranged in an iterative manner, and the output of the AND gates are connected with Full Adders. For the design of a 4x4 Braun Array Multiplier we need a total of **9 Full Adders, 16 Logic AND gates and a 3-bit Kogge-Stone Adder**. The partial products at each stage are generated by the AND Gates. Each of these partial products are added with the sum of the previously produced partial products by the row of full adders. The carry out is shifted one bit either to the left or to the right and then added to the sum which was generated by the first adder and the earlier generated partial product. The last stage of shifting needs a paralle-prefix adder hence we use the 3-bit Kogge Stone Adder. The circuit design and the corresponding symbol can be found below, along with the netlist.
